...
function generate_truck_paths() {
  const trips = _await(gtfs.getTruckTripsOrderedByTime());
  ...
  trips.forEach((trip, tripIndex) => {
    ...
    const tripPoints = [];
    const stopInfo = _await(gtfs.getTruckStopInfoForTrip(trip.trip_id));
    const stops = [];
    ...
    const response = _await(googleMapsClient.directions(request).asPromise())
      .json;
